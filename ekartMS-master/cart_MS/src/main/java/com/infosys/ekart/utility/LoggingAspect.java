...
public class LoggingAspect {
	...
	@AfterThrowing(pointcut = "execution(* com.infosys.ekart.cart.repository.*.*(..))", throwing = "exception")
	public void logExceptionFromRepository(Exception exception) throws Exception {
		log(exception);
	}

	@AfterThrowing(pointcut = "execution(* com.infosys.ekart.cart.service.*.*(..))", throwing = "exception")
